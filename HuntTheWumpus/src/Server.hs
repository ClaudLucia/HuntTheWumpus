...
handleCmd addr url req | ((stage input) == "welcome") = if ((value input) == "y")
                                                                then return $ sendText OK (handleMove roomNum 5 ++ (stage input)) 
...
-- submitGuess :: StatusCode -> String -> Response JSON
...
sendText :: StatusCode -> String -> Response JSON
sendText s v = insertHeader HdrContentLength (show (length txt))
             $ insertHeader HdrContentEncoding "UTF-8"
             $ insertHeader HdrContentEncoding "application/json"
             $ (respond s :: Response JSON) { rspBody = txt }
  where
    -- txt = encodeString v
    txt = encodeJSON v
